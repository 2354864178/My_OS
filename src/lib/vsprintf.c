...
#define is_digit(c) ((c) >= '0' && (c) <= '9')  // 快速判断字符是否为数字
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
    ...
    repeat:
        ...
        switch (*fmt)
        {
        case '-': flags |= LEFT; goto repeat;       // 左对齐调整
        case '+': flags |= PLUS; goto repeat;       // 放加号
        case ' ': flags |= SPACE; goto repeat;      // 放空格
        case '#': flags |= SPECIAL; goto repeat;    // 是特殊转换
        case '0': flags |= ZEROPAD; goto repeat;    // 要填零(即'0')，否则是空格
        }

    // 取当前参数字段宽度域值，放入 field_width 变量中
        field_width = -1;
        if (is_digit(*fmt)) field_width = skip_atoi(&fmt);  // 如果宽度域中是数值则直接取其为宽度值

        // 如果宽度域中是字符 '*'，表示下一个参数指定宽度
        else if (*fmt == '*')
        {
            ++fmt;
            field_width = va_arg(args, int);    // 调用 va_arg 取宽度值
            if (field_width < 0)    // 若此时宽度值小于 0，则该负数表示其带有标志域 '-' 标志（左对齐）
            {
    ...
    // 取格式转换串的精度域，并放入 precision 变量中
        precision = -1;
        if (*fmt == '.')    // 精度域开始的标志是'.' 其处理过程与上面宽度域的类似
        {
            ++fmt;
            if (is_digit(*fmt)) precision = skip_atoi(&fmt);// 如果精度域中是数值则直接取其为精度值
            else if (*fmt == '*') precision = va_arg(args, int);// 如果精度域中是字符'*'，表示下一个参数指定精度
            if (precision < 0) precision = 0;// 精度为负则设为0
        }
        ...
        // 若格式转换指示符是 'n'，表示要把到目前为止转换输出的字符数保存到对应参数指针指定的位置中
        case 'n':
            // 首先利用 va_arg() 取得该参数指针
            ip = va_arg(args, int *);
        ...
        case 'b': // 二进制：直接调用number指定进制 2，实现整数转二进制字符串；
            num = va_arg(args, unsigned long);
            str = number(str, &num, 2, field_width, precision, flags);
            break;
        case 'm': // （MAC 地址）：循环处理 6 个字节，每个字节转 2 位十六进制，用:分隔：
            flags |= SMALL | ZEROPAD;
            ...
            str--;
            break;
        case 'r': // （IP 地址）：循环处理 4 个字节，每个字节转十进制，用.分隔，逻辑与 MAC 地址类似。
            flags |= SMALL;
            ptr = va_arg(args, u8 *);
